...
import deepl
import os # Still used by st.secrets if implemented, but not for local file paths here
import io # For creating in-memory Excel files for download
import json # No longer used for API keys, but good to keep if other JSON ops are added
...
# CONFIG_FILE_NAME constant removed
...
# --- API Key Management functions for local file saving/loading are removed ---
...
# Callbacks for API key changes (only update session state and configure service)
def handle_google_api_key_change():
    if "google_api_input_widget" in st.session_state:
        ...
        if st.session_state.google_api_key:
            ...
            st.session_state.google_api_configured_status = None # Key cleared or empty
...
def handle_deepl_api_key_change():
    if "deepl_api_input_widget" in st.session_state:
        ...
        # DeepL doesn't have a global configure like genai, key is used per translation call.
        # Status can be inferred by key presence.

# Initialize session state
if 'app_initialized' not in st.session_state:
    # Initialize API keys as empty strings. They will be populated by user input.
    st.session_state.google_api_key = ""
    st.session_state.deepl_api_key = ""
    st.session_state.google_api_configured_status = None
    # st.session_state.keys_saved_locally removed
    ...
    # Attempt to configure Google AI if a key is somehow pre-set (e.g., via st.secrets in future)
    # For now, this will effectively not run on initial load as keys are empty.
    if st.session_state.google_api_key:
        try:
            genai.configure(api_key=st.session_state.google_api_key)
            st.session_state.google_api_configured_status = True
...
with st.sidebar.expander("🔑 API Keys Configuration", expanded=not bool(st.session_state.get('google_api_key'))):
    # Markdown about local saving removed
    ...
    if st.session_state.get('google_api_configured_status') is True: 
        ...
        st.error(st.session_state.google_api_configured_status) # Show configuration error
...
if app_mode == "Generate Alt Text from Images":
    ...
    with st.expander("⚙️ Advanced Prompt Settings (Optional)", expanded=False):
        ...
        custom_prompt_input = st.text_area("Custom Prompt:", value=st.session_state.get('custom_prompt', DEFAULT_PROMPT),
                                           ...
                                           help="Your custom prompt will be sent with the image. For default, do not change.")
    ...
    if st.button("✨ Generate English Alt Text", type="primary", key="generate_button_main"):
        # Check if Google AI key is configured (True means successful configuration attempt)
                ...
                for i, source_item in enumerate(image_sources_to_process):
                    ...
                    try:
                        if source_type == "file":
                            ...
                            status_text_gen.text(f"Fetching & generating for: {source_item[:70]}...")
    ...
    if st.session_state.generated_results is not None and not st.session_state.generated_results.empty:
        ...
        with col1_actions:
            st.download_button(
                label="📥 Download Edited English Alt Texts (Excel)", 
                data=to_excel(st.session_state.generated_results), 
        ...
        if st.session_state.get('show_translation_options', False):
            ...
            if st.button("🔄 Start Translation", type="primary", key="start_translation_btn"):
                if not st.session_state.get('deepl_api_key'): # Check if DeepL key is present in session
        ...
        for i_trans_disp in range(len(df_display_trans)):
            ...
            with col2_trans_disp:
                ...
                st.text_area(f"English Alt Text (Index: {row_trans_disp['ImageIndex']}):", 
                           ...
                           height=80,
                           key=f"eng_alt_text_trans_display_{row_trans_disp['ImageIndex']}", 
                           disabled=True)
                
                for col_disp in df_display_trans.columns:
                    if col_disp.startswith("Alt Text -"): 
                        st.text_area(f"{col_disp.replace('Alt Text - ', '')}:", 
                                   value=row_trans_disp[col_disp], 
                                   height=80,
                                   key=f"trans_display_{col_disp}_{row_trans_disp['ImageIndex']}", 
                                   disabled=True)
            st.divider()
            ...
            try:
                ...
                if source_column_name not in df_excel.columns:
                    ...
                    for lang_name in selected_languages_names_excel:
                        ...
                        excel_trans_disabled_cols.append(col_name) 
    ...
    if 'excel_translated_df_display' in st.session_state and st.session_state.excel_translated_df_display is not None:
        ...
        st.data_editor(
            ...
            disabled=disabled_cols_display 
        )
...
st.sidebar.markdown("---")
st.sidebar.caption("AI Alt Text Generator & Translator")
st.sidebar.caption("https://github.com/bilal-karagoz/alt_text/")
